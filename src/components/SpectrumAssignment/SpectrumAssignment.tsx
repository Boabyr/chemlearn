  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        <p className="text-sm text-muted">Ordne jedem Peak die richtige Bezeichnung zu:</p>
        {peaks.map((peak, i) => {
          const result = results[i]
          let borderCol = 'border-line'
          if (checked) borderCol = result.correct ? 'border-success' : 'border-danger'
          else if (assignments[peak.id]) borderCol = 'border-blue-500'

          return (
            <div key={peak.id} className={`flex items-center gap-3 border rounded-xl px-4 py-3 transition-colors bg-raised ${borderCol}`}>
              ...
              <select
                ...
                disabled={checked}
                className="flex-1 bg-surface border border-line rounded-lg px-3 py-1.5 text-ink text-sm focus:outline-none focus:border-blue-400">
              ...
              {checked && (
                <span className={`text-sm font-semibold flex-shrink-0 ${result.correct ? 'text-success' : 'text-danger'}`}>
      ...
      <div className="flex gap-2 flex-wrap">
        {!checked && (
          <button onClick={check} disabled={!allAssigned}
            className={`px-5 py-2.5 rounded-xl font-semibold text-sm transition-colors ${
              allAssigned ? 'bg-blue-600 hover:bg-blue-500 text-ink' : 'bg-sunken text-subtle cursor-not-allowed'
            }`}>
            Zuordnung prüfen ✓
          </button>
        )}
        {checked && !allCorrect && (
          <button onClick={reset}
            className="px-5 py-2.5 rounded-xl font-semibold text-sm bg-sunken border border-line text-muted">
            ⟳ Nochmal
          </button>
        )}
        {hints < 2 && hint1 && (
      ...
      {hints >= 1 && hint1 && <div className="px-4 py-2 bg-warning/10 border border-warning rounded-lg text-xs text-warning">{hint1}</div>}
      {hints >= 2 && hint2 && <div className="px-4 py-2 bg-warning/10 border border-warning rounded-lg text-xs text-warning">🔍 {hint2}</div>}
    </div>
  )
}
